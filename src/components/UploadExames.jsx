...
export default function UploadExames() {
  ...
  useEffect(() => {
    try {
      ...
      if (!anamneseStatus) {
        alert('Você precisa completar a anamnese antes de fazer upload dos exames.');
      ...
      alert('Erro ao carregar dados. Recarregando a página...');
    ...
    try {
      ...
      const arquivosValidos = novosArquivos.filter(arquivo => {
        ...
        if (arquivo.size > maxSize) {
          ...
          alert(`Arquivo ${arquivo.name} é muito grande. Tamanho máximo: 10MB`);
          return false;
        }
      ...
      if (arquivosValidos.length > 0) {
        ...
        console.log(`${arquivosValidos.length} arquivo(s) adicionado(s) com sucesso`);
      }
      
      if (arquivosValidos.length !== novosArquivos.length) {
        const rejeitados = novosArquivos.length - arquivosValidos.length;
        alert(`${rejeitados} arquivo(s) foram rejeitados. Verifique o tipo e tamanho dos arquivos.`);
      }
    } catch (error) {
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        setUploading(false);
        alert('Exames enviados com sucesso! Agora você pode solicitar a análise IA.');
        navigate('/menu');
      }, 1500);
      ...
      setUploading(false);
      alert('Erro ao fazer upload dos exames. Tente novamente.');
    }
  };
